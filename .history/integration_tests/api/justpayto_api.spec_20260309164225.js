...
// ─── Helper: Get token with one automatic retry on failure ───────────────────
//     Use this in beforeAll hooks instead of accessing body.access_token directly.
//
//     Behaviour:
//       • On first attempt: if the response is valid and has access_token → return it.
//       • On failure (500, non-JSON, missing token): wait 2 seconds, try once more.
//       • If the retry also fails → throw a clear error that skips the describe block
//         with a human-readable diagnosis instead of crashing on null.

async function getTokenOrSkip(request) {
    async function attempt(attemptNumber) {
        console.log(`🔄 Token generation attempt ${attemptNumber}…`);
        const { response, body } = await generateToken(request);
        const status = response.status();

        if (body && body.access_token) {
            console.log(`✅ Token obtained on attempt ${attemptNumber} (HTTP ${status})`);
            return body.access_token;
        }

        // Return null so the caller can decide whether to retry or fail
        console.warn(
            `⚠️  Attempt ${attemptNumber} failed — HTTP ${status}.`,
            body ? `Body: ${JSON.stringify(body)}` : 'Response was not JSON.'
        );
        return null;
    }

    // ── First attempt ──────────────────────────────────────────────────────
    const firstToken = await attempt(1);
    if (firstToken) return firstToken;

    // ── Wait 2 s then retry once ───────────────────────────────────────────
    console.warn('⏳ Waiting 2 s before retry…');
    await new Promise(resolve => setTimeout(resolve, 2000));

    const retryToken = await attempt(2);
    if (retryToken) return retryToken;

    // ── Both attempts failed — fail with a descriptive message ─────────────
    throw new Error(
        `❌ Token generation failed after 2 attempts. All tests in this group are skipped.\n` +
        `   BASE_URL    : ${BASE_URL}\n` +
        `   username    : ${CREDENTIALS.username}\n` +
        `   apiUsername : ${CREDENTIALS.apiUsername}\n` +
        `   → Verify credentials, apiUsername, and that the sandbox is reachable.`
    );
}
...
test.describe('🔐 Authentication — /access-token/generate', () => {
    ...
    test('POST generates a valid access token with correct credentials', async ({ request }) => {
        ...
        // Provide a clear failure message when the server returns a non-200 status
        expect(
            response.status(),
            `❌ Expected 200 but got ${response.status()}. ` +
            (response.status() === 500
                ? 'Server error — sandbox may be down or credentials are invalid.'
                : 'Check BASE_URL, username, password, and apiUsername.')
        ).toBe(200);
...
test.describe('💳 Payment Methods — /payment-methods', () => {
    ...
    test.beforeAll(async ({ request }) => {
        accessToken = await getTokenOrSkip(request);
...
test.describe('💰 Create Payment — /payment/create', () => {
    ...
    test('POST returns 464 when using an already-expired payment method', async ({ request }) => {
        ...
        const freshToken = await getTokenOrSkip(request);
