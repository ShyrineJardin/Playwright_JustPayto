...
export async function checkEmail(options) {
  ...
  try {
    ...
    if (email) {
      ...
      console.log(`   Body length: ${JSON.stringify(email.body).length} characters`);
    } else {
      console.log('❌ No email found matching criteria');
    }
    
    return email;
  } catch (error) {
    console.error('❌ Gmail check error:', error.message);
    console.error(`   Credentials: ${credentialPathUser}`);
    console.error(`   Token: ${tokenPathUser}`);
    
    // Return null instead of throwing to allow tests to handle
    return null;
  }
}
