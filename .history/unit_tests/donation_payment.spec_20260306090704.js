...
test.beforeAll(async ({browser}) => {
    const paymentURL = process.env.GAWADKALINGA_PAYMENT_URL;

    if (!paymentURL) {
        throw new Error('❌ GAWADKALINGA_PAYMENT_URL is not defined in environment variables');
    }
...
test.describe('🎨 Donation Payment UI Page Tests', () => {
    ...
    test('should display correct payment page with matching slug', async ({ page }) => {
        ...
        throw new Error(`Invalid GAWADKALINGA_PAYMENT_URL: ${paymentUrl}`);
        }
...
test.describe('⚠️ Error Message Validation for empty fields', () => {
    test.beforeEach(async ({ page }) => {
        const paymentUrl = process.env.GAWADKALINGA_PAYMENT_URL;
        if (!paymentUrl){
        throw new Error('❌ Environment variable GAWADKALINGA_PAYMENT_URL is not set');
        }
    ...
    test('Empty Message Field should show error message', async({page}) => {
        ...
        console.log('👆 Clicking "Donate Now" button with empty message')
    ...
    test('Empty Payment Method field should show error message', async ({page}) => {
        ...
        await page.locator('#what-is-your-donation-for-add-a-message-or-additional-notes').fill('Test Message for unit testing');
    ...
    test ('Empty Amount field should show error message', async ({page}) => {
        ...
        console.log('👆 Clicking "Donate Now" button with empty amount')
