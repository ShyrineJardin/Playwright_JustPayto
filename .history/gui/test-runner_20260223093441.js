...
app.get('/api/zap-status', (req, res) => {
  ...
  request.on('error', () => {
    res.json({ reachable: false, host, port, message: `ZAP not reachable at ${host}:${port}. Make sure ZAP is running.` });
  });

  request.on('timeout', () => {
    request.destroy();
    res.json({ reachable: false, host, port, message: `ZAP connection timed out at ${host}:${port}.` });
  });
});
