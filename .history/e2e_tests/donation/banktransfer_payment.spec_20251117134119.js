...
test('🤲 Bank Transfer payment for Donation', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    try {
        ...
        await page.goto(process.env.GAWADKALINGA_PAYMENT_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    } catch (error) {
        console.error('❌ Failed to load page:', error.message);
        console.log('⏳ Attempting to wait for page content...');
        // Try to wait for a key element instead of full page load
        try {
            await page.locator('button:has-text("Donate Now")').waitFor({ state: 'visible', timeout: 30000 });
