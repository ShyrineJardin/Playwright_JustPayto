...
test('🤲 Online Bank payment for Donation', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    console.log('💻 Complete Online Bank Payment Flow for donation');
    console.log(`🔗 URL: ${process.env.GAWADKALINGA_PAYMENT_URL}`);

    try {
        // Increase timeout to 90 seconds for slow network/GUI conditions
        await page.goto(process.env.GAWADKALINGA_PAYMENT_URL, { waitUntil: 'domcontentloaded', timeout: 90000 });
    } catch (error) {
    ...
    console.log('⏳ Waiting for Donate Now button to be visible...');
    ...
    await expect(page).toHaveURL(/justpay\.to/);    console.log('💸 Click "Donate Now" button')
    ...
    const testMessage = 'OnlineBankDonationTest12345';
    ...
    await expect(page.getByText('OnlineBankDonationTest12345')).toBeVisible();
    ...
    console.log('🎉 Donation Online Bank Payment Flow Test Completed Successfully');
    ...
    from: 'hello@justpay.to',
    to: process.env.INDIVIDUAL_USER_EMAIL,
    subject: 'Complete your donation',
    wait_time_sec: 30,
