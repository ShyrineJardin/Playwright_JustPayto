...
test('🤲 Credit Card payment for Donation', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    const cardnumError = await page.locator('body').textContent();

    if (cardnumError.includes('Card Number is required')) {
    console.log('✅ Error message for card number appeared as expected');
    } else {
    const bodyTextLower = cardnumError.toLowerCase();
    if (bodyTextLower.includes('card number is required')) {
        console.log('✅ Error message for card number appeared (lowercase match)');
    } else {
        console.log(`Current page text: ${cardnumError}`);
...
const bodyText5 = await page.locator('body').textContent();

if (bodyText5.includes('Expiration Date (MM/YY) is required')) {
  console.log('✅ Error message for expiration date appeared as expected');
} else {
  const bodyTextLower = bodyText5.toLowerCase();
  if (bodyTextLower.includes('expiration date (mm/yy) is required')) {
    ...
    console.log(`Current page text: ${bodyText5}`);
