...
console.log('⏳ Waiting for Load button to be ready...');
    ...
    // Wait for either: loading class gone OR disabled attribute gone
    await Promise.race([
        // Option 1: Wait for loading to disappear
        page.locator('span.loading').waitFor({ state: 'hidden', timeout: 30000 }).catch(() => {}),
        
        // Option 2: Wait for button to be enabled
        page.waitForFunction(() => {
            const btn = document.querySelector('button[type="submit"]');
            return btn && !btn.disabled && !btn.querySelector('span.loading');
        }, { timeout: 30000 })
    ]);
    
    // Final check: is button enabled?
    const isDisabled = await loadButton.isDisabled();
    
    if (isDisabled) {
        console.log('⚠️ Button still disabled after waiting, force clicking...');
        await loadButton.click({ force: true });
    } else {
        console.log('✅ Button enabled, clicking normally');
