...
test('🏦 Credit card payment for business user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    const emailError2 = (await page.locator('body').innerText()).toLowerCase();
    if (!emailError2.includes('payer/sender email is required')) {
    ...
    await page.locator('button:has-text("OK")').click();
    const mobileError2 = (await page.locator('body').innerText()).toLowerCase();

    if (!mobileError2.includes('mobile number is required')) {
