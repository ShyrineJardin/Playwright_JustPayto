...
test('🏦 Bank transfer payment for business user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    test.setTimeout(250000); // 3 minutes to accommodate processing time

    console.log('⏳ Waiting for transaction to process (10 seconds)...');
    await page.waitForTimeout(10000); 

    const testTriggerTime = Date.now();
    const searchTime = new Date(testTriggerTime - 60 * 1000); 
...
console.log('⏳ Waiting for transaction to process (10 seconds)...');
    ...
    // Calculate search time RIGHT BEFORE closing the success dialog
    // This ensures we capture emails sent after payment completion
    const testTriggerTime = Date.now();
    const searchTime = new Date(testTriggerTime - 120 * 1000); // Look back 2 minutes to be safe
    ...
    // Give email system time to process and send
    await page.waitForTimeout(10000); // 10 seconds initial wait
    ...
    const merchantEmail = await checkMerchantEmail({
        ...
        wait_time_sec: 30, // Check every 30 seconds
        max_wait_time_sec: 180, // Wait up to 3 minutes
        after: searchTime.toISOString(), // Use same search time
    });
