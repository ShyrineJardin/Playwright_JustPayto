...
test('🏦 Online Bank transfer payment for business user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    // Calculate search time BEFORE clicking confirm
    const testTriggerTime = Date.now();
    const searchTime = new Date(testTriggerTime - 60 * 1000); // Look back 1 minute
    ...
    // Email verification for user with proper delay
    console.log('📧 Verifying payment confirmation email for business user');
    console.log('⏳ Waiting 5 seconds for email system to process...');
    await page.waitForTimeout(5000); // 5 second delay to allow email system to send

    console.log('📬 Checking inbox for confirmation email...');
