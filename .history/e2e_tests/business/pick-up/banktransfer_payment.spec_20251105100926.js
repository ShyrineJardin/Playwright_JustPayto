...
test('🏦 Bank transfer payment for business user - pickup', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    const testMessage = 'BankTransferBusinessPickUpTest12345';
    ...
    await expect(page.getByText('BankTransferBusinessPickUpTest12345')).toBeVisible();
    ...
    test.setTimeout(250000); // Extend timeout to accommodate processing time

    console.log('⏳ Waiting for transaction to process (10 seconds)...');
    ...
    await page.screenshot({ path: `banktransfer_business_${browserName}.png`, fullPage: true });
    ...
    // Email verification for user with pick up time 
    ...
    Here's the fixed code with validation for both emails - the payment confirmation and the pickup ready notification:
javascript// Calculate search time RIGHT BEFORE closing success dialog
    const testTriggerTime = Date.now();
    const searchTime = new Date(testTriggerTime - 120 * 1000); // Look back 2 minutes
    ...
    // Email verification for user - Payment Confirmation
    console.log('📧 Verifying payment confirmation email for business user');
    console.log('📬 Waiting for confirmation email for payer...');
    ...
    console.log('✅ Payment confirmation email received successfully');
    ...
    // Verify the pickup time is mentioned in the email (09:00 start time)
    const emailBody = pickupEmail.body || pickupEmail.html || '';
    if (!emailBody.includes('9:00') && !emailBody.includes('09:00')) {
        console.log('⚠️ Warning: Pickup start time (9:00) not found in email body');
    } else {
        console.log('✅ Pickup start time (9:00) verified in email');
    }
