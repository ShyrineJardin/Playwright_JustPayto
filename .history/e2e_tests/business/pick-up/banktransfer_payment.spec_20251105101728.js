...
test('🏦 Bank transfer payment for business user - pickup', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    if (!pickupEmail) {
     ...
     // Verify pickup time (09:00) is mentioned in the email body
    console.log('🕒 Verifying pickup start time in email content...');
    if (pickupEmail.body) {
        const emailBody = JSON.stringify(pickupEmail.body);
        // Check for 09:00 (with leading zero as it appears in the email)
        const hasPickupTime = emailBody.includes('09:00');
        
        if (!hasPickupTime) {
            console.log('⚠️ Warning: Pickup start time (09:00) not found in email body');
            console.log('📄 Email body preview:', emailBody.substring(0, 300));
        } else {
            console.log('✅ Pickup start time (09:00) verified in email content');
        }
        
        // Also verify the pickup time range
        const hasTimeRange = emailBody.includes('09:00 - 17:00');
        if (hasTimeRange) {
            console.log('✅ Full pickup time range (09:00 - 17:00) verified in email');
        }
    } else {
        console.log('⚠️ Warning: Email body is empty, cannot verify pickup time');
    }
