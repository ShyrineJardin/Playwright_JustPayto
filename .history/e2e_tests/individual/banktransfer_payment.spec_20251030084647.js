...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    if (otpPrompt.includes('sent to your mobile') && !otpPrompt.includes('email')) {
        ...
        const otpCodeMatch = email.body.match(/(\d{6})/);
        const otp = otpCodeMatch ? otpCodeMatch[0] : null;

        if (otp) {
            await page.locator('#otp-input').fill(otp);
            console.log(`✅ OTP code ${otp} entered successfully`);
            await page.click('#veri')
