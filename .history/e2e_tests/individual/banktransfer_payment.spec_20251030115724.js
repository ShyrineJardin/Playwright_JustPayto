...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    console.log('🔗 Redirecting to MoneyBank mock page ...')
    await page.waitForURL(/bank-web-mock\.xendit\.co/);

    const [popup] = await Promise.all([
        context.waitForEvent('page'),
        page.locator('button', { hasText: 'MoneyBank' }).click()
    ]);

    await popup.waitForLoadState();
    console.log(`✅ MoneyBank mock page loaded: ${popup.url()}`);
    ...
    if (!successMessage.toLowerCase().includes('was successful')) {
    ...
    console.log('📧 Email received for payer');
    console.log(`✅ Payer email verified: ${process.env.INDIVIDUAL_USER_EMAIL}`);
    
    // Verify email timestamp is recent (within last 5 minutes)
    if (payerEmail.internalDate) {
    const emailTime = parseInt(payerEmail.internalDate);
    const timeDiff = testTriggerTime1 - emailTime;

    if (timeDiff > 5 * 60 * 1000) {
        console.log(`⚠️ Warning: Email is ${Math.round(timeDiff / 1000)} seconds old`);
        throw new Error('Email received is too old - may be from previous test');
    }

    console.log(`✅ Email timestamp verified (${Math.round(timeDiff / 1000)} seconds old)`);
    }

    // --- Email verification for merchant ---
    console.log('📬 Waiting for confirmation email in merchant inbox...');
    await page.waitForTimeout(1500);

    const testTriggerTime2 = Date.now();
    console.log('⏰ Test trigger time:', new Date(testTriggerTime2).toISOString());

    const searchTime2 = new Date(testTriggerTime2 - 30 * 1000);

    const merchantEmail = await checkEmail({
    from: 'hello@justpay.to',
    ...
    console.log('📧 Email received on merchant side');
    console.log(`✅ Merchant email verified: ${process.env.INDIVIDUAL_MERCHANT_EMAIL}`);

    // Verify merchant email timestamp
    if (merchantEmail.internalDate) {
    const emailTime = parseInt(merchantEmail.internalDate);
    const timeDiff = testTriggerTime2 - emailTime;

    if (timeDiff > 5 * 60 * 1000) {
        console.log(`⚠️ Warning: Merchant email is ${Math.round(timeDiff / 1000)} seconds old`);
        throw new Error('Merchant email received is too old - may be from previous test');
    }

    console.log(`✅ Merchant email timestamp verified (${Math.round(timeDiff / 1000)} seconds old)`);
    }

    console.log('✅ Email verification for payer and merchant completed successfully.');
