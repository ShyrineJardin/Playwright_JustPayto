...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    // Wait for the OTP instruction element to appear
    await page.waitForSelector('p.MuiTypography-body1', { timeout: 10000 });

    const otpInstruction = await page.locator('p.MuiTypography-body1').innerText();
    console.log('📄 OTP instruction text:', otpInstruction);

    if (otpInstruction.toLowerCase().includes('sent to your mobile') && !otpInstruction.toLowerCase().includes('email') ) {
