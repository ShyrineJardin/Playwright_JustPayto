...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    if (isSmsOtp) {
        ...
        await Promise.race([
            page.waitForURL(/payment|success|summary/, { timeout: 120000 }), // Wait for navigation
            page.locator('button[type="submit"]').click() // Or wait for submit button to be clickable
        ]);
