...
test('📱 eWallet payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {

    console.log('💻 Complete eWallet Payment Flow');
    ...
    const payNowButton = page.locator('button:has-text("Send Money")');
    const isButtonVisible = await payNowButton.isVisible({timeout: 30000}).catch(() => false);
    
    if (!isButtonVisible) {
        console.error('❌ FAILED: Individual Payment page did not load properly');
    ...
    const testMessage = 'eWalletTest12345';
    ...
    //payment page contact information for verification
        ...
        if (isSmsOtp) {
            ...
            test.setTimeout(200000); // 2 minutes total
    ...
    await expect(page.getByText('eWalletTest12345')).toBeVisible();
    ...
    console.log('🎉 Individual eWallet Payment Flow Test Completed Successfully');
