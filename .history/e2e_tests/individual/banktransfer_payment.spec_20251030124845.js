...
console.log('📸 Screenshot saved as debug-after-continue.png');

// Try to find MoneyBank button with more flexible selector
const moneyBankButton = page.locator('button:has-text("MoneyBank"), button:has-text("Money Bank"), a:has-text("MoneyBank")');
const buttonCount = await moneyBankButton.count();
console.log(`🔢 Found ${buttonCount} MoneyBank button(s)`);

if (buttonCount === 0) {
    console.log('⚠️ MoneyBank button not found. Looking for alternative selectors...');
    ...
    throw new Error('❌ MoneyBank button not found on page');
}

console.log('🏦 MoneyBank button found, waiting for it to be clickable...');
await moneyBankButton.first().waitFor({ state: 'visible', timeout: 10000 });

console.log('🏦 Opening MoneyBank mock page popup...')
const [popup] = await Promise.all([
    context.waitForEvent('page'),
    moneyBankButton.first().click()
]);
