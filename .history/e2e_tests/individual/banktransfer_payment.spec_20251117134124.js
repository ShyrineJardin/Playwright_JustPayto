...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    try {
        // Increase timeout to 60 seconds for slow network conditions
        await page.goto(process.env.INDIVIDUAL_PAYMENT_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    } catch (error) {
        console.error('❌ Failed to load page:', error.message);
        console.log('⏳ Attempting to wait for page content...');
        // Try to wait for a key element instead of full page load
        try {
            await page.locator('button:has-text("Send Money")').waitFor({ state: 'visible', timeout: 30000 });
            console.log('✅ Page content eventually loaded');
        } catch (innerError) {
            console.error('❌ Page still not loaded after extended wait');
            throw error;
        }
    }
