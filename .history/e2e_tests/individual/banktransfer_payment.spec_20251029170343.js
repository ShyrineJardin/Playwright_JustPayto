...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    await page.locator('body').innerText().then(async (bodyText) => {
        if (body.locator('#your-residential-address').length > 0 && bodyText.toLowerCase().includes('residential address is required')) {
            cy.log('🔐 KYC section detected - Processing KYC fields');
            ...
            const fileInput = page.locator('input[type="file"].FileUpload').first();


            //uploading government ID using fixtures/test-image.jpg
            ...
            const fileInfo = await fileInput.evaluate((input) => {
            ...
            if (fileInfo) {
                console.log(`✅ Government ID attached: ${fileInfo.name} (${fileInfo.sizeMB} MB)`);
            } else {
            ...
            if (bodyText.includes('government-issued id is required')) {
            ...
            //check for esignature error
            const esignError = (await page.locator('body').innerText()).toLowerCase(); 
            if (!esignError.includes('e-signature is required')) {
                throw new Error('❌ Esignature error message not displayed');
            } else {
                console.log('✅ Esignature error message displayed as expected');
            }

            //filling in esignature
            console.log('✍️ Uploading in e-signature');
            await fileInput.setInputFiles(filePath);
            console.log('✅ Esignature uploaded successfully');
            ...
            if (fileInfo) {
                console.log(`✅ Esignature attached: ${fileInfo.name} (${fileInfo.sizeMB} MB)`);
            } else {
                console.log('⚠️ Could not verify file attachment');
            }
            ...
            // Submit form
            console.log('📨 Submitting form with esignatire');
            await page.getByText('OK').click();
