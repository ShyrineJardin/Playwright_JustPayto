...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    // Get all text content from the page to check for OTP instructions
    const pageContent = await page.locator('body').innerText();
    const pageBodyText = pageContent.toLowerCase();
    console.log('📄 Checking page content for OTP type...');
    
    // Log a snippet of the body text for debugging
    console.log('📄 Body text snippet:', pageBodyText.substring(0, 200));
    
    // Check if it's SMS OTP (matches Cypress logic exactly)
    // SMS OTP: contains "sent to your mobile" AND does NOT contain "and/or email" AND does NOT contain "email"
    const isSmsOtp = (
        pageBodyText.includes('sent to your mobile') && 
        !pageBodyText.includes('and/or email') && 
        !pageBodyText.includes('email')
    );
    
    console.log(`📊 Detection results - SMS OTP: ${isSmsOtp}`);
    ...
    if (isSmsOtp) {
        ...
        console.log('⏸️ Test Pause - Please enter the OTP manually to continue the test');

        await page.locator('input#enter-the-code')

        console.log('🔃 Testing is not done yet, wait 20 secs to continue after entering the OTP');
        await page.waitForTimeout(20000);

        console.log('✅ Resuming test after manual OTP entry');
