...
test('🏦 Bank Transfer payment for individual user', async ({page, context, baseURL, browserName, playwright}) => {
    ...
    await page.locator('body').innerText().then(async (bodyText) => {
        if (body.locator('#your-residential-address').length > 0 && bodyText.toLowerCase().includes('payer/sender residential address is required')) {
            ...
            if (!nationalityError.includes('payer/sender nationality is required')) {
                throw new Error('❌ National    ity error message not displayed');
            }           
