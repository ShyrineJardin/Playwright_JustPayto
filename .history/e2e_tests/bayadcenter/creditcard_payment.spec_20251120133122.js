...
async function runBayadCenterTest(page, billerConfig, context) {
    ...
    const otherFeesValue = parseFloat(otherFees.replace(/[₱,]/g, ''));

    // Verify Total Amount and calculation
    console.log('💰 Verifying Total Amount and Fee Calculation');
    const totalAmountRow = page.locator('.MuiTable-root tbody tr', { hasText: 'Total Amount' });
    const totalAmount = await totalAmountRow.locator('td').nth(1).innerText();
    console.log(`✅ Total Amount displayed: ${totalAmount}`);
    expect(totalAmount).toMatch(/₱[\d,]+\.\d{2}/);

    // Extract total amount value
    const totalAmountMatch = totalAmount.match(/₱([\d,]+\.\d{2})/);
    const totalAmountValue = totalAmountMatch ? parseFloat(totalAmountMatch[1].replace(/,/g, '')) : 0;

    // Calculate expected total
    const calculatedTotal = basePaymentValue + processingFeeValue + systemFeeValue + otherFeesValue;
    console.log(`🧮 Calculated total: ₱${calculatedTotal.toFixed(2)}`);
    console.log(`📊 Displayed total: ₱${totalAmountValue.toFixed(2)}`);

    // Verify calculation is correct (allowing for small rounding differences)
    expect(Math.abs(totalAmountValue - calculatedTotal)).toBeLessThan(0.01);
    console.log('✅ Total amount calculation verified correctly');

    // Verify additional fees indicator
    const totalFeesAdded = processingFeeValue + systemFeeValue + otherFeesValue;
    console.log(`📈 Total additional fees: ₱${totalFeesAdded.toFixed(2)}`);
    if (totalAmount.includes('+')) {
        const additionalFeesMatch = totalAmount.match(/\+(\d+\.\d{2})/);
        if (additionalFeesMatch) {
            const displayedAdditionalFees = parseFloat(additionalFeesMatch[1]);
            console.log(`✅ Additional fees indicator: +${displayedAdditionalFees}`);
            expect(Math.abs(displayedAdditionalFees - totalFeesAdded)).toBeLessThan(0.01);
        }
    }
