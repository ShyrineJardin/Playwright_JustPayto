...
const BILLER_CONFIGS = {
     'cignal': {
        ...
        selectors: {
            accountNumber: 'input[name="referenceNumber"]',  // Note: uses referenceNumber, not accountNumber
            amount: 'input#amount[name="amount"]',
            lastName: 'input[name="LastName"]',
            firstName: 'input[name="FirstName"]',
            middleInitial: 'input[name="MI"]'
        },
        // Optional: Add radio button selector if needed
        radioButton: {
...
const BILLERS_TO_TEST = ['cignal']; // Only test these
...
async function runBayadCenterTest(page, billerConfig, context) {
    ...
    const rawDisplayedAmount = await amountInput.inputValue();
    const expectedAmount = billerConfig.fields.amount;

    // Normalize both values
    const displayedNum = parseFloat(rawDisplayedAmount.replace(/,/g, ''));
    const expectedNum = parseFloat(expectedAmount);

    console.log(`  Expected amount: ${expectedNum}`);
    console.log(`  Displayed raw: ${rawDisplayedAmount}`);
    console.log(`  Displayed parsed: ${displayedNum}`);

    if (displayedNum === expectedNum) {
        console.log('✅ Amount matches successfully');
    } else {
        throw new Error(`❌ Amount mismatch: Expected ${expectedNum} but got ${rawDisplayedAmount}`);
    }
