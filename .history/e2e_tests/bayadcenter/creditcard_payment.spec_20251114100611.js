...
test('💳 Creditcard Payment for BayadCenter', async({page, context, baseURL, browserName, playwright})=> {
  ...
  // Check if the page actually loaded with content
    ...
    if (!isButtonVisible) {
        ...
        console.error('❌ "Pay Now" button not found - The page may be down or loading slowly');
        await page.screenshot({ path: `bayadcenter-load-error-${Date.now()}.png` });
        throw new Error('BayadCenter page failed to load - Pay Now button not visible');
    } else {
